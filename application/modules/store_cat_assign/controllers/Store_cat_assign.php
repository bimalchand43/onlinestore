...
function submit($item_id){
    ...
    if($submit == "Finished"){
        ...
        if($cat_id != ""){
            ...
            $flash_msg = "The Item was successfully assigned to ".$cat_title." category!!";
            $value = '<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
            $this->session->set_flashdata('item', $value);
...
function update($item_id){
    ...
    //get an array of all subcategories on the site
    $this->load->module('store_categories');
    $sub_categories = $this->store_categories->_get_all_sub_cats_for_dropdown();
       
    //get an array of all assigned categories on the site
