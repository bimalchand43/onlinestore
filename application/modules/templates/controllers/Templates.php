...
class Templates extends MX_Controller{
	...
	function _draw_page_top(){
		...
		$this->_draw_top_lhs();
		$this->_draw_top_mhs($shopper_id);
		$this->_draw_top_rhs($shopper_id);
	}

	function _draw_top_mhs($shopper_id){
		$this->load->module('site_security');
		$shopper_id = $this->site_security->_get_user_id();
		...
		if((is_numeric($shopper_id)) AND ($shopper_id>0)){
			...
			$view_file = 'page_top_mid_out'; //user Not logged in
		}
		$this->load->view($view_file);
	}

	function _draw_top_rhs($shopper_id){
	...
	function _draw_breadcrumbs($data){
		...
		$this->load->view('breadcrumbs_public_frontend', $data);
	}
	...
	function public_frontend($data){
		...
		$this->load->view('public_frontend', $data);
	}
