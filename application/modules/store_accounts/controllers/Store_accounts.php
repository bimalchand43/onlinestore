...
function _get_shopper_address($update_id, $delimiter){
    //return the address of the shopper
...
function delete($update_id){
 if(!is_numeric($update_id)){
    ...
    if($submit=="Cancel"){
        ...
        $flash_msg = "The store account was successfully deleted!!";
...
function update_pword(){
    ...
    if($submit == "Submit"){
        ...
        if($this->form_validation->run()==TRUE){
            ...
            //update the password or user details
...
function create(){
    ...
    if($submit == "Submit"){
        ...
        if($this->form_validation->run()==TRUE){
            ...
            if(is_numeric($update_id)){
                $this->_update($update_id, $data);
                $flash_msg = "The Account was successfully Updated!!";
                $value = '<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
                $this->session->set_flashdata('item', $value);
                redirect('store_accounts/create/'.$update_id); 
            }else{
    ...
    if((is_numeric($update_id)) && ($submit!="submit")){
     ...
     if(!is_numeric($update_id)){
        ...
        $data['headline'] = "Update New Account";
    }
    $data['update_id'] = $update_id;
    $data['flash'] = $this->session->flashdata('item');
