...
function _send_auto_notify($update_id){
    //Note: notify customer when an order status has updated
...
function _get_order_status_title(){
    //note: this gets called by /browse and it figures out the order status title
    $this->load->module('store_order_status');
...
function submit_order_status(){
    ...
    if($submit == "Cancel"){
        //get current order status for this order
        $query = $this->get_where($update_id);
        foreach ($query->result() as $row) {
            $order_status = $row->order_status;
        }
...
function view(){
    ...
    //fetch the contents of the shopping cart/basket
...
function browse(){
    ...
    //count the orders that belongs to this order status
    $use_limit = FALSE;
