...
function _generate_pagination($data){
	...
	if($template == "public_frontend"){
		$settings = $this->get_settings_public_frontend();
...
function get_settings_public_frontend(){
...
function get_settings_for_admin(){
	...
	$settings['full_tag_open'] = '<nav aria-label="Page navigation example"><ul class="pagination justify-content-center">';
	$settings['full_tag_close'] = '</ul></nav>';
...
/*function get_settings_for_some_other_template(){
	$settings['num_links'] = '';
