...
function deleteconf($update_id){
    if(!is_numeric($update_id)){
        ...
        //prevent them from deleting home and contactus
        redirect('site_security/not_allowed');
    }
    $this->load->module('site_security');
    $this->site_security->_make_sure_is_admin();
...
function create(){
    ...
    if($submit == "Submit"){
        ...
        if($this->form_validation->run()==TRUE){
            ...
            if(is_numeric($update_id)){
                ...
                $flash_msg = "The Web Page was successfully Updated!!";
                $value = '<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
                $this->session->set_flashdata('item', $value);
                redirect('webpages/create/'.$update_id); 
            }else{
                ...
                $flash_msg = "The web Page was successfully Created!!";
                $value = '<div class="alert alert-success" role="alert">'.$flash_msg.'</div>';
                $this->session->set_flashdata('item', $value);
                redirect('webpages/create/'.$update_id); 
    ...
    if((is_numeric($update_id)) && ($submit!="submit")){
     ...
     if(!is_numeric($update_id)){
        $data['headline'] = "Add New Webpage";
    }else{
        $data['headline'] = "Update New Webpage";
    }
    $data['update_id'] = $update_id;
    $data['flash'] = $this->session->flashdata('item');
