...
function _avoid_cart_conflicts($data){
    ...
    */
    ...
    $orginal_session_id = $data['session_id'];
    $shopper_id = $data['shopper_id'];

    $col1 = 'session_id';
    $value1 = $orginal_session_id;
    $col2 = 'shopper_id';
    $value2 = $shopper_id;
    ...
    if($num_rows>0){//items conflicting with store_shoppertrack!
        ...
        //update/change the data['session_id'] variable
...
function add_to_basket(){
   ...
   if($submit == "Submit"){
        ...
        $this->form_validation->set_rules('iteam_size', 'Item Size', 'numeric');
...
function _fetch_the_data(){
    //gathers together all of data, so that we can do table insert 
    $this->load->module('site_security');
    $this->load->module('store_items');

    $item_id = $this->input->post('item_id', TRUE);
    $item_data = $this->store_items->fetch_data_from_db($item_id);
    $item_price = $item_data['iteam_price'];
    ...
    $data['item_title'] = $item_data['iteam_title'];
...
function _get_value($value_type, $update_id){
    ...
    //Note: value-TYPE cane be 'color' or 'size'
    if($value_type == 'size'){
